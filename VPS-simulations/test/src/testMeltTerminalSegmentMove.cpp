...
/**
 * Tests for PolymerMeltCBMCSampler::generateForwardTerminalSegmentMove(). 
 */
TEST_CASE(
...
)
{
    ...
    // --------------------------------------------------------------- //
    // Terminal segment moves on 10-mers with no angle or dihedral potentials 
    // --------------------------------------------------------------- //
    const int length = 10;
    ...
    // Try generating a collection of 3-mer terminal segment moves at the
    // head of chain 4
    ...
    for (int i = 0; i < n_moves; ++i)
    {
        auto result = sampler_random.generateForwardTerminalSegmentMove(
            chain_idx, segment_length, TerminalSegmentEnd::HEAD, n_candidates
        );
        ...
        for (int j = 0; j < segment_length; ++j)
        {
            ...
            // ... and check using periodic distances 
            found_r_new_j = false;
            for (int k = 0; k < n_candidates; ++k)
            {
                Matrix<double, 3, 1> dvec = periodicDistVec<double>(
                    r_new_j, candidates(k, Eigen::seqN(3 * j, 3)),
                    xlen, ylen, zlen
                );
                if (dvec.norm() < tol)
                {
        ...
        // configuration
        Matrix<double, 3, 1> r_end = coords[chain_idx].row(segment_length); 
        REQUIRE((r_new.row(0) - r_end.transpose()).norm() < fene_params["R0"]);
        REQUIRE(
            periodicDistVec<double>(r_new.row(0), r_end, xlen, ylen, zlen).norm() < fene_params["R0"]
        ); 
        ...
        for (int j = 1; j < segment_length; ++j)
        {
            ...
            REQUIRE(periodicDistVec<double>(r1, r2, xlen, ylen, zlen).norm() < fene_params["R0"]);  
        }
        ...
        for (int j = 0; j < segment_length; ++j)
        {
            ...
            if (j == 0)
                predecessor = coords[chain_idx].row(segment_length); 
            else 
            ...
            for (int k = 0; k < n_candidates; ++k)
            {
                ...
                r = periodicDistVec<double>(predecessor, candidates(k, Eigen::seqN(3 * j, 3)), xlen, ylen, zlen).norm(); 
                REQUIRE(r < fene_params["R0"]);  
            } 
        }
        ...
        for (int j = 0; j < segment_length; ++j)
        {
            ...
            for (int k = 0; k < n_candidates; ++k)
            {
                ...
                {
                    Matrix<double, 3, 1> dvec = periodicDistVec<double>(
                        r_curr, coords[chain_idx].row(segment_length),
                        xlen, ylen, zlen
                    );
                    double r = dvec.norm();  
                    residual_ijk += lj<double>(
    ...
    {
        auto result = sampler_random.generateForwardTerminalSegmentMove(
            chain_idx, segment_length, TerminalSegmentEnd::TAIL, n_candidates
        );
        Matrix<double, Dynamic, Dynamic> candidates = std::get<0>(result); 
        Matrix<double, Dynamic, 3> r_new = std::get<1>(result);  
        double log_forward_rosenbluth = std::get<2>(result);
        ...
        // configuration
        Matrix<double, 3, 1> r_end = coords[chain_idx].row(length - 1 - segment_length); 
        REQUIRE((r_new.row(0) - r_end.transpose()).norm() < fene_params["R0"]); 
        REQUIRE(
            periodicDistVec<double>(r_new.row(0), r_end, xlen, ylen, zlen).norm() < fene_params["R0"]
        ); 
        ...
        for (int j = 0; j < segment_length; ++j)
        {
            Matrix<double, 3, 1> predecessor; 
            if (j == 0)
                predecessor = coords[chain_idx].row(length - 1 - segment_length); 
            else 
                predecessor = r_new.row(j - 1);
            for (int k = 0; k < n_candidates; ++k)
            {
        ...
        for (int j = 0; j < segment_length; ++j)
        {
            ...
            for (int k = 0; k < n_candidates; ++k)
            {
                ...
                {
                    Matrix<double, 3, 1> dvec = periodicDistVec<double>(
                        r_curr, coords[chain_idx].row(length - 1 - segment_length),
                        xlen, ylen, zlen
                    ); 
                    double r = dvec.norm(); 
                    residual_ijk += lj<double>(
    ...
    // --------------------------------------------------------------- //
    // Terminal segment moves on 10-mers with a cosine angle potential and no 
    // dihedral potential 
    // --------------------------------------------------------------- //
    ...
    // --------------------------------------------------------------- //
    // Terminal segment moves on 10-mers with a cosine angle potential and a
    ...
    // --------------------------------------------------------------- //
    // Terminal segment moves on 10-mers with a Gaussian angle potential and a
    // harmonic dihedral potential 
    // --------------------------------------------------------------- //
    melt_config = generateKMerMelt<double>(
...
/**
 * Tests for PolymerMeltCBMCSampler::getBackwardTerminalSegmentMoveRosenbluthWeight().
 */
TEST_CASE(
    "Tests for backward Rosenbluth factor calculation for terminal segment moves",
...
)
{
    ...
    for (int i = 0; i < n_moves; ++i)
    {
        ...
        Matrix<double, Dynamic, 3> r_new = std::get<1>(result);
        PolymerMeltConfiguration<double> melt_config_moved(melt_config); 
        melt_config_moved.replaceSegment(chain_idx, r_new.colwise().reverse(), 0);
        Matrix<double, Dynamic, 3> coords_moved = melt_config_moved.getSegment(chain_idx, 0, length);
        ...
        for (int j = segment_length; j < length; ++j)
        {
            ...
            // is fine here 
            REQUIRE_THAT(
                (coords_moved.row(j) - coords[chain_idx].row(j)).norm(),
                Catch::Matchers::WithinAbs(0, tol)
            );  
        }
        ...
        { 
            ...
            // is fine here 
            REQUIRE_THAT(
                (candidates(0, Eigen::seqN(3 * j, 3)) - coords[chain_idx].row(segment_length - 1 - j)).norm(),
                Catch::Matchers::WithinAbs(0, tol)
            ); 
        } 
        ...
        for (int j = 0; j < segment_length; ++j)
        {
            ...
            predecessor = coords[chain_idx].row(segment_length - j); 
            for (int k = 0; k < n_candidates; ++k)
            {
                double r = (predecessor.transpose() - candidates(k, Eigen::seqN(3 * j, 3))).norm();
                REQUIRE(r < fene_params["R0"]); 
        ...
        for (int j = 0; j < segment_length; ++j)
        {
            ...
            for (int k = 0; k < n_candidates; ++k)
            {
                ...
                {
                    Matrix<double, 3, 1> dvec = periodicDistVec<double>(
                        r_curr, coords_moved.row(m), xlen, ylen, zlen
                    ); 
                    double r = dvec.norm();
                    residual_ijk += lj<double>(
                        r, lj_params["eps"], lj_params["sigma"], true
                ...
                {
                    Matrix<double, 3, 1> dvec = periodicDistVec<double>(
                        r_curr, coords_moved.row(segment_length),
                        xlen, ylen, zlen
                    );
                    double r = dvec.norm();  
                    residual_ijk += lj<double>(
                ...
                {
                    Matrix<double, 3, 1> dvec = periodicDistVec<double>(
                        r_curr, coords[chain_idx].row(segment_length - 1 - m),
                        xlen, ylen, zlen
                    );
                    double r = dvec.norm(); 
                    residual_ijk += lj<double>(
    ...
    for (int i = 0; i < n_moves; ++i)
    {
        ...
        Matrix<double, Dynamic, 3> r_new = std::get<1>(result);
        PolymerMeltConfiguration<double> melt_config_moved(melt_config); 
        melt_config_moved.replaceSegment(chain_idx, r_new, length - segment_length); 
        Matrix<double, Dynamic, 3> coords_moved = melt_config_moved.getSegment(chain_idx, 0, length);
        ...
        { 
            REQUIRE_THAT(
                (candidates(0, Eigen::seqN(3 * j, 3)) - coords[chain_idx].row(length - segment_length + j)).norm(), 
                Catch::Matchers::WithinAbs(0, tol)
            ); 
        } 
        ...
        for (int j = 0; j < segment_length; ++j)
        {
            ...
            // original configuration 
            predecessor = coords[chain_idx].row(length - segment_length - 1 + j);
            for (int k = 0; k < n_candidates; ++k)
            {
                double r = (predecessor.transpose() - candidates(k, Eigen::seqN(3 * j, 3))).norm();
                REQUIRE(r < fene_params["R0"]); 
        ...
        for (int j = 0; j < segment_length; ++j)
        {
            ...
            for (int k = 0; k < n_candidates; ++k)
            {
                ...
                {
                    Matrix<double, 3, 1> dvec = periodicDistVec<double>(
                        r_curr, coords_moved.row(length - segment_length - 1),
                        xlen, ylen, zlen
                    ); 
                    double r = dvec.norm(); 
                    residual_ijk += lj<double>(
                ...
                {
                    Matrix<double, 3, 1> dvec = periodicDistVec<double>(
                        r_curr, coords[chain_idx].row(length - segment_length + m),
                        xlen, ylen, zlen
                    );
                    double r = dvec.norm(); 
                    residual_ijk += lj<double>(
...
TEST_CASE("Tests for terminal segment moves", "[moveOnce()]")
{
    ...
    for (int i = 0; i < n_moves; ++i)
    { 
        // Try moving by choosing from 10000 candidate 3-mer terminal segment
        // moves on chain i % 5 ... 
        //
        ...
        auto result = sampler_random.moveOnce(
            n_candidates, chain_idx, CBMCMoveType::TERMINAL_SEGMENT, segment_length
        ); 
        ...
        if (terminal_end == TerminalSegmentEnd::HEAD)
            melt_config_moved.replaceSegment(
                chain_idx, forward_moves.colwise().reverse(), 0
            );
        else 
            melt_config_moved.replaceSegment(
                chain_idx, forward_moves, length - segment_length
            );
        Matrix<double, Dynamic, 3> coords_moved = melt_config_moved.getSegment(chain_idx, 0, length); 
