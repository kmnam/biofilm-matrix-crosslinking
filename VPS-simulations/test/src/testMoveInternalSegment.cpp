...
TEST_CASE("Tests for internal segment move generation", "[generateInternalSegmentMoves()]")
{
    ...
    auto result = sampler_cosine.generateInternalSegmentMoves(
        n_candidates, 4, 3, max_iter, init_tangent_stepsize, min_tangent_stepsize
    );
    ...
    Matrix<int, Dynamic, 1> found_long_bond = Matrix<int, Dynamic, 1>::Zero(n_candidates); 
    for (int i = 0; i < n_candidates; ++i)
    {
    ...
    // Try generating 50 5-atom internal segment moves for the segment [1, 2, 3, 4, 5]
    // with a new seed (for later comparison)
    sampler_gaussian.seed(1234567890); 
    result = sampler_gaussian.generateInternalSegmentMoves(
        n_candidates, 5, 1, max_iter, init_tangent_stepsize, min_tangent_stepsize
    );
    r_new = result.first; 
    ...
    // Check that, for any new segment along which there is a bond that is 
    // too long, the corresponding energy difference is infinite
    found_long_bond = Matrix<int, Dynamic, 1>::Zero(n_candidates); 
    for (int i = 0; i < n_candidates; ++i)
    {
    ...
    sampler_gaussian.seed(1234567890); 
    result = sampler_gaussian.generateInternalSegmentMoves(
        n_candidates, 5, 1, max_iter, init_tangent_stepsize / 2,
        min_tangent_stepsize
    );
    Matrix<double, Dynamic, Dynamic> r_new_small = result.first; 
    Matrix<double, Dynamic, 1> energy_diffs_small = result.second;
    REQUIRE(r_new_small.rows() == n_candidates);
    REQUIRE(r_new_small.cols() == 15);  
    REQUIRE(energy_diffs_small.size() == n_candidates);
    ...
    // Check that the total perturbation of each segment is smaller
    for (int i = 0; i < n_candidates; ++i)
    {
        // Get the sum of the perturbations of all atoms along each segment 
        double sum1 = 0; 
        double sum2 = 0;
        for (int j = 0; j < 5; ++j)
        {
...
TEST_CASE("Tests for internal segment moves", "[moveOnce()]")
{
    ...
    auto result_cosine = sampler_cosine.moveOnce(
        n_candidates, CBMCMoveType::INTERNAL_SEGMENT, 4, internal_move_params
    ); 
    ...
    if (proposed_new_move)
    {
        ...
        Matrix<double, Dynamic, 1> forward_diffs(n_candidates); 
        for (int i = 0; i < n_candidates; ++i)
        { 
        ...
        for (int i = 0; i < 4; ++i)
        {
            Matrix<double, 3, 1> r1 = reverse_moves(move_idx, Eigen::seqN(3 * i, 3));
    ...
    auto result_gaussian = sampler_gaussian.moveOnce(
        n_candidates, CBMCMoveType::INTERNAL_SEGMENT, 5, internal_move_params
    ); 
